...
public class OcorrenciaSpecification {
    ...
    return (root, query, cb) -> {
            ...
            if (seletor.getSala() != null){
                predicates.add((Predicate) cb.like(cb.lower(root.get("sala")), "%" +  seletor.getSala().toLowerCase() + "%"));
            }

            if (seletor.getCategoria() != null){
                predicates.add((Predicate) cb.like(cb.lower(root.get("categoria")), "%" +  seletor.getCategoria().toLowerCase() + "%"));
            }
            ...
            if (seletor.getDataInicio() != null && seletor.getDataFim() != null) {
                ...
                predicates.add(cb.between(root.get("dataOcorrencia"), seletor.getDataInicio(),
                        seletor.getDataFim()));
